...
# Chirp and system parameters
START_FREQUENCY = 0.05       # Hz
END_FREQUENCY = 3.0         # Hz
CHIRP_DURATION = 37.5       # s
POSITION_BANDWIDTH = 0.6    # Hz (-3 dB bandwidth of the example model)
CHIRP_AMPLITUDE = 1.0

# Keep equal to CHIRP_DURATION for a real-time animation.
CHIRP_RUN_TIME = 37.5
INTRO_WAIT = 2.0
FINAL_WAIT = 3.0
...
class PositionHoldChirp(Scene):
    """Visualise a horizontal position-hold chirp and the drone response."""
    ...
    def construct(self):
        ...
        position_command = CHIRP_AMPLITUDE * np.sin(phase)

        # First-order closed-loop approximation of position hold.
        position_response = np.zeros_like(position_command)
        dt = 1 / sample_rate
        pole = 2 * np.pi * POSITION_BANDWIDTH
        alpha = 1 - np.exp(-pole * dt)
        for k in range(1, len(position_response)):
            position_response[k] = (
                position_response[k - 1]
                + alpha * (position_command[k] - position_response[k - 1])
            )
        ...
        # ------------------------------------------------------------------
        title = Text(
            "Position-hold chirp: effect of increasing frequency",
            font_size=34,
            color=INK,
            weight="SEMIBOLD",
        ...
        parameter_text = VGroup(
            Text("Start frequency: 0.05 Hz", font_size=17, color=BLUE),
            Text("Poshold bandwidth: approximately 0.6 Hz", font_size=17, color=ORANGE),
            Text("Final frequency: 3 Hz", font_size=17, color=RED),
            Text("Chirp duration: 75 s", font_size=17, color=INK),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.10)
        parameter_text.to_corner(UR, buff=0.55).shift(DOWN * 0.65 + RIGHT * 0.35)
        ...
        command_label = VGroup(
            ...
            Text("Position command", font_size=18, color=BLUE),
        ).arrange(RIGHT, buff=0.12)
        response_label = VGroup(
            Line(ORIGIN, RIGHT * 0.45, color=ORANGE, stroke_width=4),
            Text("Drone position", font_size=18, color=ORANGE),
        ).arrange(RIGHT, buff=0.12)
        ...
        response_graph = axes.plot_line_graph(
            x_values=time,
            y_values=position_response,
            add_vertex_dots=False,
            line_color=ORANGE,
            stroke_width=3.2,
        )

        # Drone moves horizontally. The vertical blue line is the commanded
        # position, while the orange point marks the actual position.
        asset_path = Path(__file__).with_name("drone_clean.png")
        drone = ImageMobject(str(asset_path))
        drone.set_width(4.7)
        drone_base = LEFT * 0.8 + UP * 0.1
        ...
        reference_line = always_redraw(
            lambda: DashedLine(
                LEFT * (
                    0.8 - position_scale
                    * value_at(position_command, tracker.get_value())
                ) + DOWN * 1.10,
                LEFT * (
                    0.8 - position_scale
                    * value_at(position_command, tracker.get_value())
        ...
        reference_label = always_redraw(
            lambda: Text("commanded position", font_size=17, color=BLUE)
            .next_to(reference_line, UP, buff=0.10)
            .shift(DOWN * 0.15)
        )
        actual_position_dot = always_redraw(
            lambda: Dot(
                drone_base
                + RIGHT * position_scale
                * value_at(position_response, tracker.get_value())
                + DOWN * 1.0,
                radius=0.075,
                color=ORANGE,
            )
        ...
        dynamic_readout = always_redraw(
            lambda: VGroup(
                ...
                Text(
                    ...
                    color=(
                        GREEN
                        if frequency_at(tracker.get_value()) < 0.7 * POSITION_BANDWIDTH
                        else ORANGE
                        if frequency_at(tracker.get_value()) <= 1.3 * POSITION_BANDWIDTH
                        else RED
                    ),
                ),
        ...
        self.play(
            ...
            FadeIn(reference_line), FadeIn(reference_label),
            FadeIn(actual_position_dot), FadeIn(drone),
        ...
        conclusion = VGroup(
            ...
            Text("The drone follows the position command.", font_size=16, color=INK),
            Text("Above the bandwidth", font_size=16, color=RED, weight="SEMIBOLD"),
            Text("The horizontal motion is attenuated and delayed.", font_size=16, color=INK),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.10)

        # Position der Textgruppe
    ...
    def make_status(self, current_frequency):
        if current_frequency < 0.7 * POSITION_BANDWIDTH:
            text = "The drone closely follows the command"
            color = GREEN
        elif current_frequency <= 1.3 * POSITION_BANDWIDTH:
            text = "Near the bandwidth: attenuation and phase lag"
            color = ORANGE
        else:
            text = "Above the bandwidth: motion is attenuated"
            color = RED
