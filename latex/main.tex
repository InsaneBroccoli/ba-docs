...
The basis of the flight control system is the angular rate controller, which regulates the rotational speed of the drone around its axes. It is also the innermost control loop, which directly influences the other control loops, such as the angle and position hold. Therefore, the plant model of the angular rate controller serves as the fundamental reference for the analysis of the entire control system.

As you can see in figure~\ref{fig:signal_processing_model}, the plant of the angular rate control loop  includes the influence of the different voltages applied to the motors, up to the measured angular rate.

\paragraph{Motor Dynamics}

The dynamics of the motor–propeller system can be described using the well-established electromechanical model of a DC motor, which is also applicable to brushless DC (BLDC) motors in an equivalent form. The model consists of an electrical and a mechanical differential equation.

The electrical behavior of the motor is given by
\begin{equation}
...
\begin{equation}
J\,\frac{d\omega(t)}{dt} = k_t\,i(t) 
\label{eq:dc_motor_mechanical}
\end{equation}
where $J$ denotes the combined moment of inertia of the motor and propeller and $k_t$ the torque constant. The viscous friction and the Load Torque are neglected for simplicity.

Out of these equations, the transfer function from the applied voltage to the angular velocity can be derived as
\begin{equation}
G(s) = \frac{\omega(s)}{u(s)} = \frac{k_t}{Js(Ls + R)}.
\end{equation}

This transfer function describes a bode plot with a slope of -20dB at the beginning, which corresponds to the integrator behavior of the motor. At higher frequencies, the slope increases to -40dB and an -180° phase shift due to the inductive effects of the motor windings.
...
The position hold has the angular rate and the angle as inner control loops. Therefor the plant of the position hold is the transformation from the angle of the drone to its position in space. This transformation is determined by the kinematics of the multirotor and the influence of external forces such as gravity.

The position of the drone can be described by its coordinates in a three-dimensional space, typically denoted as \( (x, y, z) \). The angle of the drone, which corresponds to its orientation, influences how the thrust generated by the motors is directed, and thus how the drone moves in space.
If we consider that the drone is hovering at a certain altitude, we are able to calculate the acceleration of the drone in the horizontal direction as a function of the angle and the thrust. To Hover the thrust must be equal to the weight of the drone, which is given by \( mg \), where \( m \) is the mass of the drone and \( g \) is the acceleration due to gravity. The horizontal acceleration can then be expressed as

\begin{equation}
a_{hor}(t) = g \cdot \sin(\theta(t)),
\end{equation}

If we assume that during the position hold the drone operates at small angles, we can apply the small-angle approximation, which states that \( \sin(\theta) \approx \theta \) for small values of \( \theta \). This leads to a simplified expression for the horizontal acceleration:
\begin{equation}
a_{hor}(t) \approx g \cdot \theta(t).
\end{equation}

By integrating the horizontal acceleration twice with respect to time, we can derive the position of the drone in the horizontal plane as a function of the angle:
\begin{equation}
x(t) = \int \int a_{hor}(t) \, dt^2
\approx \int \int g \cdot \theta(t) \, dt^2.
\end{equation}
Applying the Laplace transform to this expression, we obtain the transfer function from the angle to the position:
\begin{equation}
P_{\text{Position}}(s) = \frac{X(s)}{\Theta(s
)} = \frac{g}{s^2}.
\end{equation}

With this transfer function, we can expect a bode plot of the position plant with a slope of -40 dB/decade and a phase shift of -180 degrees, which are characteristic features of a double integrator.
